...
(function ColourfulAmbient() {
  ...
  function injectElements() {
    ...
    // Colourful orbs
    const orbDefs = [
      { section:'about',    color:'rgba(255,107,107,0.55)', size:500, top:'5%',  left:'60%', delay:0 },
      { section:'about',    color:'rgba(245,200,66,0.5)',  size:350, top:'55%', left:'10%', delay:5 },
      { section:'reviews',  color:'rgba(167,139,250,0.5)', size:380, top:'15%', left:'70%', delay:4 },
      { section:'benefits', color:'rgba(56,189,248,0.5)',  size:420, top:'20%', left:'75%', delay:2 },
      { section:'benefits', color:'rgba(45,212,191,0.45)', size:300, top:'60%', left:'20%', delay:8 },
      ...
      { section:'logoInterstitial', color:'rgba(255,107,107,0.4)', size:360, top:'20%', left:'15%', delay:2 },
      { section:'logoInterstitial', color:'rgba(167,139,250,0.4)', size:340, top:'70%', left:'85%', delay:5 },
    ];
    orbDefs.forEach(def => {
  ...
  function initStarField() {
    ...
    const layers = [
      { count:220, sizeRange:[0.3,0.8],  speed:0.008, alpha:[0.10,0.30] },
      { count: 90, sizeRange:[0.6,1.4],  speed:0.018, alpha:[0.15,0.45] },
      { count: 30, sizeRange:[1.0,2.2],  speed:0.032, alpha:[0.25, 0.6] },
    ];
      ...
      layers.forEach((l, li) => {
        for (let i = 0; i < l.count; i++) {
          stars.push({
            ...
            hue: [ '255,255,255', '255,200,200', '200,220,255', '255,230,160' ][Math.floor(Math.random()*4)]
          });
        }
      });
    }
    ...
    function draw() {
      ...
      grad.addColorStop(0.3,'#04000a');
      grad.addColorStop(0.6,'#06000f');
      grad.addColorStop(1,'#000000');
      ctx.fillStyle = grad; ctx.fillRect(0,0,W,H);

      const glow1 = ctx.createRadialGradient(W*0.25,H*0.35,0,W*0.25,H*0.35,W*0.6);
      glow1.addColorStop(0,'rgba(255,107,107,0.030)');
      glow1.addColorStop(0.5,'rgba(245,200,66,0.018)');
      glow1.addColorStop(1,'transparent');
      ctx.fillStyle = glow1; ctx.fillRect(0,0,W,H);

      const glow2 = ctx.createRadialGradient(W*0.75,H*0.6,0,W*0.75,H*0.6,W*0.45);
      glow2.addColorStop(0,'rgba(56,189,248,0.025)');
      ...
      stars.forEach(s => {
        ...
        if(s.r>1.4){
          ...
          glow.addColorStop(0,`rgba(${s.hue},${(alpha*0.18).toFixed(3)})`);
  ...
  function initShootingStars() {
    function spawnStar() {
      ...
      const colors = ['255,255,255','255,200,150','200,230,255','255,220,255'];
      const color  = colors[Math.floor(Math.random()*colors.length)];
      el.style.cssText = `left:${startX}px;top:${startY}px;opacity:0;transform:rotate(${angle}deg);background:rgb(${color});`;
      const tail = document.createElement('div');
      tail.style.cssText = `position:absolute;top:50%;right:0;transform:translateY(-50%);height:1px;background:linear-gradient(90deg,transparent,rgba(${color},0.7));width:0;transition:width ${dur*0.4}ms ease;`;
      el.appendChild(tail);
  ...
  function injectDividers() {
    // showcaseSection intentionally excluded — has its own ::before ground-gradient.
    const secs = ['#about','#benefits','#products','#quality','#for-whom','#contact','#reviews','#coming-soon'];
    secs.forEach(sel=>{
