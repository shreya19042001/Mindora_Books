...
const BOOKS = [
  {
    ...
    isbn:'ISBN 9789358139754',
    cover:'images/b1-cover.png', backCover:'images/b1-p6.png',
    bgImage:'images/gallery-6.png',
    pageCount:'56 Pages · 25+ Coloring Pages',
    pages:[
    ...
    feats:['56 premium thick pages, 25+ coloring designs','Ganesha, Krishna, Hanuman, Durga & more','Educational & culturally enriching','Perfect festival gift'],
    bg:'#0d0020',
    accent:'#ffd700',
    accentRgb:'255,215,0',
  ...
  },
  {
    ...
    feats:['56 premium thick pages, 25+ coloring designs','Elephant, monkey, giraffe, lion & more','Vibrant jungle & river-themed scenes','Promotes creativity & love for wildlife'],
    bg:'#001a08',
    accent:'#34d399',
  ...
  },
  {
    ...
    isbn:'ISBN 9789358955286',
    cover:'images/b3-cover.png', backCover:'images/b3-p6.png',
    bgImage:'images/gallery-4-sea.png',
    pageCount:'56 Pages · 25+ Coloring Pages',
    pages:[
    ...
    feats:['56 premium thick pages, 25+ coloring designs','Gold fish, seahorse, dolphin, jellyfish & more','Big, bold & easy designs','Boosts creativity and focus — Ages 5–15'],
    bg:'#000d20',
    accent:'#38bdf8',
...
/* ══════════════════════════════════════════
   HIDE NAVBAR CONTACT LINK NEAR CONTACT/FOOTER
══════════════════════════════════════════ */
(function initNavContactVisibility(){
  const navContactLinks = document.querySelectorAll('.nav-contact-link');
  const contactSection  = document.getElementById('contact');
  if(!navContactLinks.length || !contactSection) return;

  const obs = new IntersectionObserver(entries => {
    entries.forEach(entry => {
      navContactLinks.forEach(link => {
        link.style.transition = 'opacity 0.3s ease, visibility 0.3s ease';
...
function applyInfoPanel(idx, animate){
  ...
  const buyMenu  = document.getElementById('cBuyMenu');
  ...
  setTimeout(() => {
    ...
    if(isbn)   isbn.textContent = b.isbn; // FIX: dashes removed at source in BOOKS data
    ...
    if(buyMenu){
      const L = b.links;
      buyMenu.innerHTML =
        `<a href="${L.amazon}"   target="_blank" class="menu-amz">${SVG_ICONS.amz} Amazon</a>
         <a href="${L.flipkart}" target="_blank" class="menu-fk">${SVG_ICONS.fk}  Flipkart</a>
         <a href="${L.meesho}"   target="_blank" class="menu-ms">${SVG_ICONS.ms}  Meesho</a>
         <a href="${L.whatsapp}" target="_blank" class="menu-wa">${SVG_ICONS.wa}  WhatsApp</a>`;
    }
...
function applyBackground(idx){
  ...
  // Themed illustrated backdrop image per book
  const bgImg = document.getElementById('cBgImage');
  if(bgImg && b.bgImage){
    bgImg.style.opacity = '0';
    setTimeout(() => {
      bgImg.src = b.bgImage;
      bgImg.style.opacity = '0.28';
    }, 350);
  }
}
...
function toggleBuyMenu(){
  ...
  const wrap = document.querySelector('.c-discover-wrap');
  if(wrap) wrap.classList.toggle('open', buyMenuOpen);
}
...
document.addEventListener('click', e => {
  const wrap = document.querySelector('.c-discover-wrap');
  if(!wrap) return;
  if(!wrap.contains(e.target)) closeBuyMenu();
});
...
/* ════════════════════════════════════════════
   FIX: Hide navbar social icons (WhatsApp, IG, FB, Email)
   when #contact or footer section is in view
   Add: <script src="app-navicons-fix.js"></script>
   AFTER app.js (and app-cosmic.js if present)
════════════════════════════════════════════ */
(function(){
  function init(){
    const navbar = document.getElementById('navbar');
    ...
    if(!navbar || (!contact && !footer)) return;

    const targets = [contact, footer].filter(Boolean);

    const obs = new IntersectionObserver(entries => {
      // If ANY observed target (contact or footer) is intersecting, hide icons
      ...
      // Recompute properly: check all targets' current state
      const shouldHide = targets.some(t => {
        const r = t.getBoundingClientRect();
        return r.top < window.innerHeight * 0.9 && r.bottom > 0;
      });

      navbar.classList.toggle('hide-social', shouldHide);
    }, { threshold: 0.01 });

    targets.forEach(t => obs.observe(t));

    // Also run on scroll as a safety net (cheap rect check)
    window.addEventListener('scroll', () => {
      const shouldHide = targets.some(t => {
        const r = t.getBoundingClientRect();
        return r.top < window.innerHeight * 0.9 && r.bottom > 0;
